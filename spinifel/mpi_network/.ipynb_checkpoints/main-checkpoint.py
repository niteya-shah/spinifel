...
if settings.use_network_prior:
    import torch
    from .network import network_estimation
...
def main():
    ...
    logger.log("In MPI main with network prior {_network_prior}".format(_network_prior="ON" if settings.use_network_prior else "OFF"))
    if settings.use_psana:
        ...
        if (
            ...
            if generation == 0:
                ...
                if (
                    ...
                    save_mrc(settings.out_dir / f"rho-reference.mrc", reference)
                ...
                if settings.use_network_prior:
                    # Get the network prior
                    network_prior_func = network_estimation.network_estimation(slices_, device='cuda' if torch.cuda.is_available() else 'cpu')
                    real_mesh = network_estimation.get_real_mesh(settings.M, np.max(pixel_distance_reciprocal), np)
                    rho_estimated = network_prior_func(real_mesh.reshape(-1,3)).reshape(real_mesh.shape[:3])
                    
                    rho_estimated_ = np.fft.ifftshift(rho_estimated)
                    mask = rho_estimated_ > (rho_estimated_.max() * 0.1)
                    rho_estimated_ = rho_estimated_ * mask
                    save_mrc(settings.out_dir / f"rho-network-estimation-0.mrc", rho_estimated)
                    image.show_volume(np.fft.ifftshift(rho_estimated_), settings.Mquat, "rho_phased_network_0.png")
                    intensities_phased_ = np.abs(np.fft.fftn(rho_estimated_)) ** 2
                    image.show_volume(
                        np.fft.fftshift(intensities_phased_),
                        settings.Mquat,
                        "intensities_phased_network_0.png",
                    )
                    ac_phased_estimated_ = np.abs(np.fft.ifftn(intensities_phased_))
                    ac_phased_estimated = np.fft.fftshift(ac_phased_estimated_)

                    ## Save the prior
                    # if comm.rank == writer_rank:
                    #     save_mrc(settings.out_dir / f"prior-{generation}.mrc", network_prior)
                    # return
            ############################################################################
            
            if settings.use_network_prior and (generation > 0) and (generation < 4):
                network_ratio = 1 - (generation - 1) / 3
                logger.log(f"combine {network_ratio:.3f} network estimation at generation {generation:2d}.")

                rho_estimated, _, _ = align_volumes(
                    np.fft.ifftshift(rho_estimated_),
                    np.fft.ifftshift(rho_),
                    n_search=420,
                    n_iterations=10
                )
                rho_estimated_ = np.fft.fftshift(rho_estimated)
                save_mrc(settings.out_dir / f"rho-network-estimation-0-aligned.mrc", rho_estimated)
                ac_phased_estimated, _, _ = align_volumes(
                    ac_phased_estimated,
                    ac_phased,
                    n_search=420,
                    n_iterations=10
                ...
                ac_phased_estimated = ac_phased_estimated / ac_phased_estimated.sum() * ac_phased.sum()
                ac_phased = (ac_phased * (1-network_ratio) + ac_phased_estimated * network_ratio).clip(0, None)

                # Slice and Orientation matching
                orientations = snm.slicing_and_match(ac_phased)
                # orientations = snm.slicing_and_match(ac_phased_estimated)
                ...
                rho_estimated_ = np.fft.fftshift(rho_estimated)
                rho_estimated_ = rho_estimated_ / rho_estimated_.sum() * rho_.sum()
                rho_ = (rho_ * (1-network_ratio) + rho_estimated_ * network_ratio).clip(0, None)
                save_mrc(settings.out_dir / f"rho-network-combined-{generation}.mrc", np.fft.fftshift(rho_))

                ac_phased, support_, rho_ = phase(generation, ac, support_, rho_)

                logger.log(f"Problem phased in {timer.lap():.2f}s.")

                image.show_volume(np.fft.fftshift(rho_), settings.Mquat, f"rho_phased_network_{generation}.png")
