...
def setup_linops(
    ...
    ugrid_conv = autocorrelation.adjoint(
        np.ones_like(data), H_, K_, L_, M_ups, use_reciprocal_symmetry, support=None
    )
    ...
    def W_matvec(uvect):
        ...
        uvect_FDF = fourier_reg(
            uvect, ac_support, F_antisupport, M, use_recip_sym=use_reciprocal_symmetry
        )
        uvect = alambda * uvect_ADA + rlambda * uvect + flambda * uvect_FDF
        return uvect

    W = LinearOperator(dtype=np.complex64, shape=(Mtot, Mtot), matvec=W_matvec)
...
def solve_ac(
    ...
    N = np.prod(
        slices_.shape
    )  # N images per rank x number of pixels per image = number of pixels per rank
    ...
    if comm.rank == (2 if settings.use_psana else 0):
        idx = np.abs(L) < reciprocal_extent * 0.01
    ...
    image.show_volume(
        ac, settings.Mquat, f"autocorrelation_{generation}_{comm.rank}.png"
    )
