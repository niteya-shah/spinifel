...
from ..utils import getLogger, fully_qualified_module_name, SilentLogger
...
def main():
    ...
    if comm.rank == writer_rank:
        ...
        mpi_logger = SilentLogger()

    mpi_logger.info("In MPI main")
    mpi_logger.debug("Using psana")
    mpi_logger.info(f"comm.size : {comm.size:d}")
    mpi_logger.info(f"#workers  : {N_big_data_nodes:d}")
    mpi_logger.info(f"writerrank: {writer_rank}")
    mpi_logger.info(f"batch_size: {batch_size}")
    mpi_logger.info(f"max_events: {max_events}")
    ...
    mpi_logger.info(f"Loaded in {timer.lap():.2f}s.")
    ...
    if settings.load_gen > 0: # Load input from previous generation
        ...
        logger.info(f"Loading checkpoint: {checkpoint.generate_checkpoint_name(settings.out_dir, settings.load_gen, settings.tag_gen)}", flush=True)
        ...
        curr_gen = 0
        mpi_logger.info(f"#"*27)
        mpi_logger.info(f"##### Generation {curr_gen}/{N_generations} #####")
        mpi_logger.info(f"#"*27)
        ac = solve_ac(
            curr_gen, pixel_position_reciprocal, pixel_distance_reciprocal, slices_)
        mpi_logger.info(f"AC recovered in {timer.lap():.2f}s.")

        ac_phased, support_, rho_ = phase(curr_gen, ac)
        mpi_logger.info(f"Problem phased in {timer.lap():.2f}s.")
    ...
    for generation in range(curr_gen, N_generations):
        mpi_logger.info(f"#"*27)
        mpi_logger.info(f"##### Generation {generation}/{N_generations} #####")
        mpi_logger.info(f"#"*27)
        # Orientation matching
        orientations = match(
            ac_phased, slices_,
            pixel_position_reciprocal, pixel_distance_reciprocal)
        mpi_logger.info(f"Orientations matched in {timer.lap():.2f}s.")
        ...
        if settings.chk_convergence:
            ...
            if comm.rank == 0:
                ...
                prev_cov_xy = None
                cov_xy = None
            mpi_logger.info(f"CC in {timer.lap():.2f}s. cc={cov_xy:.2f} delta={cov_xy-prev_cov_xy:.2f}")
    ...
    mpi_logger.info(f"Results saved in {settings.out_dir}")
    mpi_logger.info(f"Successfully completed in {timer.total():.2f}s.")