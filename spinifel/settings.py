...
def parse_strvec(x, elt_parser):
    ...
    return tuple([elt_parser(y.strip()) for y in x[1:-1].split(",") if len(y) > 0])
...
class SpinifelSettings(metaclass=Singleton):
    ...
    def __init__(self):
        ...
        self.__properties = {
            ...
            "_show_image": ("debug", "show_image", parse_bool, True, "save png files"),
            "_data_dir": ("data", "in_dir", Path, Path(""), "data dir"),
            "_data_filename": ("data", "name", str, "", "data file name"),
            "_use_psana": ("psana", "enable", parse_bool, False, "enable PSANA"),
            "_out_dir": ("data", "out_dir", Path, Path(""), "output dir"),
            "_N_images_per_rank": (
                "runtime",
                "N_images_per_rank",
                int,
            ...
            "_N_images_max": ("algorithm", "N_images_max", int, 10000, "max images"),
            "_N_generations": (
                "algorithm",
                "N_generations",
                int,
            ...
            "_oversampling": ("algorithm", "oversampling", int, 1, "oversampling rate"),
            "_solve_ac_maxiter": (
                "algorithm",
                "solve_ac_maxiter",
                int,
            ...
            "_beta": ("algorithm", "beta", float, 0.9, "negative feedback in HIO"),
            "_cutoff": ("algorithm", "cutoff", float, 5e-2, "cutoff in shrinkwrap"),
            "_nER": ("algorithm", "nER", int, 10, "number of iterations in ER"),
            "_nHIO": ("algorithm", "nHIO", int, 5, "number of iterations in HIO"),
            "_N_phase_loops": (
                "algorithm",
                "N_phase_loops",
                int,
            ...
            "_pdb_path": ("fsc", "pdb_path", Path, Path(""), "Path for the PDB File"),
            "_fsc_zoom": (
                "fsc",
                "fsc_zoom",
                float,
            ...
            "_fsc_fraction_known_orientations": (
                "fsc",
                "fsc_fraction_known_orientations",
                float,
                0.75,
    ...
    def __init_internals(self):
        ...
        for attr in self.__properties:
            ...
            type.__setattr__(
                type(self), attr[1:], property(fget=self.__fget(attr), doc=doc)
            )
    ...
    def as_toml(self):
        ...
        for c in categories:
            ...
            for k in categories[c]:
                ...
                if p == parse_bool:
                    val = str(val).lower()
                if p in (parse_strvec_int, parse_strvec_float, parse_strvec_bool):
