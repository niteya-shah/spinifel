...
def binning_sum(arr):
    return bin2nx2n_sum(clipping(arr, settings.N_clipping), settings.N_binning)


def binning_mean(arr):
    return bin2nx2n_mean(clipping(arr, settings.N_clipping), settings.N_binning)


def binning_index(arr):
    return bin2nx2n_index(clipping_index(arr, settings.N_clipping), settings.N_binning)
...
def load_pixel_index_map(pixel_index_map):
    with h5py.File(settings.data_path, "r") as h5f:
        pixel_index_map[:] = np.moveaxis(h5f["pixel_index_map"][:], -1, 0)
