...
do
    ...
    mkdir -p $out_dir
    srun -n1 -G1 python -m spinifel --default-settings=perlmutter_network_benchmark.toml --mode=mpi_network algorithm.use_network_prior=false
    wait
done
...
do
    ...
    mkdir -p $out_dir
    srun -n1 -G1 python -m spinifel --default-settings=perlmutter_network_benchmark.toml --mode=mpi_network algorithm.use_network_prior=true
    wait
done
