...
#define assert(condition)                                                      \
  {                                                                            \
    if (!(condition)) {                                                        \
      log_lifeline_mapper.fatal(                                               \
          "%s assert %s", prolog(__FUNCTION__, __LINE__).c_str(), #condition); \
      exit(EX_SOFTWARE);                                                       \
    }                                                                          \
  }
...
public:
  LifelineMapper(MapperRuntime *rt, Machine machine, Processor local,
...
private:
  ...
  int stolenAwayTaskCount;     // tasks that are stolen away due to receiving a
                               // steal request from another processor
  int promisedFromStealsTaskCount; // tasks that are promised here by another
                                   // processor because we sent a steal request
  int promisedRelocatedTaskCount;  // tasks that are relocated here from another
                                   // processor
  ...
  int selfGeneratedTaskCount; // tasks for processor of our kind() seen here in
                              // select_task_options
  int mappedRelocatedTaskCount;     // tasks that were sent here from another
  ...
  void slice_task(const MapperContext ctx, const Task &task,
                  const SliceTaskInput &input, SliceTaskOutput &output);
  void custom_slice_task(
  ...
  bool sendSatisfiedTasks(const MapperContext ctx, SelectMappingOutput &output);
  void giveLifeThroughLifelines(const MapperContext ctx);
  void processNewReadyTasks(const SelectMappingInput &input,
                            SelectMappingOutput &output,
                            bool &mappedOrRelocated, bool &sawWorkerReadyTasks);
  ...
  bool send_stolen_tasks(MapperContext ctx, const SelectMappingInput &input,
                         SelectMappingOutput &output);
  void assignTaskId(const MapperContext ctx, const Task &task);
  ...
  void maybeGetMoreTasks(MapperContext ctx,
  ...
  void handleStealRequest(const MapperContext ctx,
                          const MapperMessage &message);
  void handleStealAck(const MapperContext ctx, const MapperMessage &message);
  void withdrawActiveLifelines(const MapperContext ctx);
  void quiesce(const MapperContext ctx);
  void handleStealNack(const MapperContext ctx, const MapperMessage &message);
...
//--------------------------------------------------------------------------
LifelineMapper::LifelineMapper(
    MapperRuntime *rt, Machine machine, Processor local,
...
//--------------------------------------------------------------------------
{
  ...
  uni = std::uniform_int_distribution<int>(0, (int)steal_target_procs.size() -
...
//--------------------------------------------------------------------------
{
  int result = promisedFromStealsTaskCount + promisedRelocatedTaskCount +
               slicedPointTaskCount + selfGeneratedTaskCount - sliceTaskCount -
               stolenAwayTaskCount - locallyEndedTaskCount;
  log_lifeline_mapper.debug(
      "%s = %d, promisedSteal %d promisedRelocated %d slicedPoint %d "
      "selfGenerated %d - slice %d stolen %d locallyEnded %d",
      prolog(__FUNCTION__, __LINE__).c_str(), result,
      promisedFromStealsTaskCount, promisedRelocatedTaskCount,
      slicedPointTaskCount, selfGeneratedTaskCount, sliceTaskCount,
      stolenAwayTaskCount, locallyEndedTaskCount);
  return result;
}
...
//--------------------------------------------------------------------------
{
  ...
  sprintf(
      buffer,
      "locallyRunning %d, totalPending %d = promisedSteal %d promisedRelocated "
      "%d slicedPoint %d selfGenerated %d - slice %d stolen %d locallyEnded %d",
      locallyRunningTaskCount(), totalPendingWorkload(),
      promisedFromStealsTaskCount, promisedRelocatedTaskCount,
      slicedPointTaskCount, selfGeneratedTaskCount, sliceTaskCount,
      stolenAwayTaskCount, locallyEndedTaskCount);
  return std::string(buffer);
...
//--------------------------------------------------------------------------
{
  ...
  sprintf(buffer, "%lld %s(%s): %s(%d)", timeNow(),
          describeProcId(local_proc.id).c_str(),
          processorKindString(local_proc.kind()), function, line);
  return std::string(buffer);
}
...
//--------------------------------------------------------------------------
{
  ...
  log_lifeline_mapper.debug("%s send STEAL_REQUEST numTasks %d to %s",
                            prolog(__FUNCTION__, __LINE__).c_str(), numTasks,
...
//--------------------------------------------------------------------------
{
  ...
  if (locallyRunningTaskCount() < MIN_RUNNING_TASKS &&
    ...
    for (std::set<const Task *>::iterator it = worker_ready_queue.begin();
      ...
      std::string desc = taskDescription(*task);
      log_lifeline_mapper.debug(
          "%s task %s should map locally, tasks_to_map_locally.size %ld",
          prolog(__FUNCTION__, __LINE__).c_str(), desc.c_str(),
...
//--------------------------------------------------------------------------
{
  if (local_proc.kind() == Processor::PY_PROC) {
    if (!quiesced) {
      ...
      log_lifeline_mapper.debug(
          "%s wantMore %d havePending %d stealOutstanding %d %s",
          prolog(__FUNCTION__, __LINE__).c_str(), wantMoreTasks,
          havePendingTasks, stealRequestOutstanding, workloadState().c_str());
      if (wantMoreTasks) {
...
//--------------------------------------------------------------------------
void LifelineMapper::handleOneStealRequest(const MapperContext ctx, Request r,
                                           bool reactivating, bool &stolen)
//--------------------------------------------------------------------------
{
  ...
  while (r.numTasks > 0 && it != worker_ready_queue.end()) {
    ...
    if (task->target_proc.kind() == r.thiefProc.kind()) {
      ...
      std::string desc = taskDescription(*task);
      log_lifeline_mapper.debug(
          "%s relocate task %s to %s %s",
          prolog(__FUNCTION__, __LINE__).c_str(), desc.c_str(),
          describeProcId(r.thiefProc.id).c_str(), workloadState().c_str());
    } else {
      it++;
    }
  }
...
//--------------------------------------------------------------------------
{
  ...
  for (std::vector<Processor>::iterator it = lifeline_neighbor_procs.begin();
    ...
    log_lifeline_mapper.debug("%s %s", prolog(__FUNCTION__, __LINE__).c_str(),
...
//--------------------------------------------------------------------------
{
  ...
  stealRequestOutstanding = false;
  log_lifeline_mapper.debug(
      "%s from %s num failed steals %d", prolog(__FUNCTION__, __LINE__).c_str(),
      describeProcId(message.sender.id).c_str(), numFailedSteals);
...
//--------------------------------------------------------------------------
{
  ...
  if (steal_target_procs.size() > 1) {
    unsigned maxProcId =
        pow(2.0, (unsigned)log2(steal_target_procs.size() - 1) + 1);
    unsigned numIdBits = (unsigned)log2(maxProcId);

    log_lifeline_mapper.debug(
        "%s maxProcId %u numIdBits %u steal_targets.size %ld",
        prolog(__FUNCTION__, __LINE__).c_str(), maxProcId, numIdBits,
        steal_target_procs.size());
    ...
    for (unsigned bit = 0; bit < numIdBits; bit++) {
      ...
      if (targetProcIndex < steal_target_procs.size()) {
        ...
        log_lifeline_mapper.info(
            "%s lifeline to %s", prolog(__FUNCTION__, __LINE__).c_str(),
...
//--------------------------------------------------------------------------
{
  ...
  unsigned long long serial = serialId != NULL ? *serialId : 0;
  sprintf(buffer, "<%s:%llx:%llx>", task.get_task_name(), serial,
          task.get_unique_id());
  return std::string(buffer);
}
...
//--------------------------------------------------------------------------
{
  ...
  int numAnalysisTasks =
      sizeof(ANALYSIS_TASK_NAMES) / sizeof(ANALYSIS_TASK_NAMES[0]);
...
{
  /// DEBUG
  log_lifeline_mapper.debug("%s address_space=%d",
                            prolog(__FUNCTION__, __LINE__).c_str(),
                            local_proc.address_space());
...
//--------------------------------------------------------------------------
{
  ...
  Rect<1, coord_t> blocks(Point<1, coord_t>(0),
                          num_blocks - Point<1, coord_t>(1));
  slices.reserve(blocks.volume());

  for (PointInRectIterator<1, coord_t> pir(blocks); pir(); pir++) {
    Point<1, coord_t> block_lo = *pir;
    Point<1, coord_t> block_hi =
        *pir + Point<1, coord_t>(TASKS_PER_STEALABLE_SLICE);

    Point<1, coord_t> slice_lo =
    ...
    for (unsigned k = 1; k < TASKS_PER_STEALABLE_SLICE; ++k)
      pir++;
  }
}

//--------------------------------------------------------------------------
void LifelineMapper::slice_task(const MapperContext ctx, const Task &task,
...
//--------------------------------------------------------------------------
{
  ...
  if (isAnalysisTask(task)) {
    ...
    std::string desc = taskDescription(task);
    log_lifeline_mapper.debug(
        "%s task %s target %s %s points %lu",
        prolog(__FUNCTION__, __LINE__).c_str(), desc.c_str(),
        describeProcId(task.target_proc.id).c_str(),
        processorKindString(task.target_proc.kind()), point_rect.volume());
    assert(input.domain.get_dim() == 1);

    Point<1, coord_t> num_blocks(point_rect.volume() /
                                 TASKS_PER_STEALABLE_SLICE);
...
void LifelineMapper::custom_slice_task(
    const Task &task, const std::vector<Processor> &local,
    const std::vector<Processor> &remote, const SliceTaskInput &input,
    SliceTaskOutput &output,
    std::map<Domain, std::vector<TaskSlice>> &cached_slices) const {
  ...
  case 2: {
    ...
    default_decompose_points<2>(point_space, procs, num_blocks,
                                false /*recurse*/, stealing_enabled,
                                output.slices);
    break;
  }
  case 3: {
    ...
    default_decompose_points<3>(point_space, procs, num_blocks,
                                false /*recurse*/, stealing_enabled,
                                output.slices);
    break;
  }
...
//--------------------------------------------------------------------------
{
  ...
  if (workerFinder != worker_ready_queue.end())
    return true;
  std::set<const Task *>::iterator relocatedFinder = relocated_tasks.find(task);
  if (relocatedFinder != relocated_tasks.end())
    return true;
  std::set<const Task *>::iterator localMapFinder =
      tasks_to_map_locally.find(task);
  if (localMapFinder != tasks_to_map_locally.end())
    return true;
  return false;
}
...
//--------------------------------------------------------------------------
{
  ...
  for (SendQueue::iterator it = send_queue.begin(); it != send_queue.end();
       it++) {
    ...
    if (tasks.size() > 0) {
      ...
      for (std::vector<const Task *>::const_iterator taskIt = tasks.begin();
        ...
        std::string desc = taskDescription(*task);
        log_lifeline_mapper.debug(
            "%s send task %s to %s", prolog(__FUNCTION__, __LINE__).c_str(),
            desc.c_str(), describeProcId(processor.id).c_str());
      }
    }
  }
  ...
  for (std::vector<std::pair<Processor, unsigned>>::iterator it =
           thiefProcs.begin();
...
//--------------------------------------------------------------------------
{
  log_lifeline_mapper.debug("%s relocated_tasks %ld "
                            ...
                            prolog(__FUNCTION__, __LINE__).c_str(),
                            relocated_tasks.size(), worker_ready_queue.size(),
...
//------------------------------------------------------------------------------
{
  std::map<Processor, Memory>::const_iterator finder = proc_sysmems.find(proc);
...
//--------------------------------------------------------------------------
{
  ...
  if (!runtime->find_or_create_physical_instance(
          ctx, target, layout_constraints, regions, result, created,
          true /*acquire*/, GC_FIRST_PRIORITY)) {
...
//--------------------------------------------------------------------------
void LifelineMapper::report_profiling(const MapperContext ctx, const Task &task,
                                      const TaskProfilingInfo &input)
//--------------------------------------------------------------------------
{
  ...
  input.profiling_responses
      .get_measurement<Realm::ProfilingMeasurements::OperationTimeline>(
          timeline);
  Realm::ProfilingMeasurements::OperationTimeline::timestamp_t elapsedNS =
      timeline.end_time - timeline.start_time;

  std::string desc = taskDescription(task);
  log_lifeline_mapper.info("%s report task complete %s %lld "
                           "totalPendingWorkload %d locallyRunning %d",
                           prolog(__FUNCTION__, __LINE__).c_str(), desc.c_str(),
                           elapsedNS, totalPendingWorkload(),
...
//--------------------------------------------------------------------------
void LifelineMapper::map_task(const MapperContext ctx, const Task &task,
                              const MapTaskInput &input, MapTaskOutput &output)
//--------------------------------------------------------------------------
{
  VariantInfo chosen =
      default_find_preferred_variant(task, ctx, true /*needs tight bound*/,
  ...
  case Processor::PY_PROC:
    ...
    break;
  default:
    log_lifeline_mapper.fatal("mapping a task onto processor type %d",
                              local_proc.kind());
  }
  output.target_procs.insert(output.target_procs.end(), procsToInsert.begin(),
                             procsToInsert.end());
  ...
  if (task.orig_proc.id == local_proc.id) {
    ...
    log_lifeline_mapper.debug("%s maps self task %s %s",
                              prolog(__FUNCTION__, __LINE__).c_str(),
                              desc.c_str(), workloadState().c_str());
  } else {
  ...
  completionRequest
      .add_measurement<Realm::ProfilingMeasurements::OperationStatus>();
  completionRequest
...
//--------------------------------------------------------------------------
void LifelineMapper::assignTaskId(const MapperContext ctx, const Task &task)
//--------------------------------------------------------------------------
{
...
//--------------------------------------------------------------------------
{
  ...
  std::string desc = taskDescription(task);
  log_lifeline_mapper.debug("print parent task %p for task %s",
                            task.parent_task, desc.c_str());

  std::string parent_desc = taskDescription(*task.parent_task);
  log_lifeline_mapper.debug(
      "%s %s on %s %s parent task %s", prolog(__FUNCTION__, __LINE__).c_str(),
      desc.c_str(), processorKindString(initial_proc.kind()),
      workloadState().c_str(),
      task.parent_task == NULL ? "<none>" : parent_desc.c_str());
 ...
 */
//------------------------------------------------------------------------
void LifelineMapper::premap_task(const MapperContext ctx, const Task &task,
...
static void create_mappers(Machine machine, Runtime *runtime,
  ...
  for (std::map<Memory, std::vector<Processor>>::iterator it =
           sysmem_local_procs->begin();
       it != sysmem_local_procs->end(); ++it)
    sysmems_list->push_back(it->first);

  for (std::set<Processor>::const_iterator it = local_procs.begin();
       it != local_procs.end(); it++) {
    LifelineMapper *mapper =
        new LifelineMapper(runtime->get_mapper_runtime(), machine, *it,
                           "lifeline_mapper", procs_list, sysmems_list,
                           sysmem_local_procs, proc_sysmems, proc_regmems);
