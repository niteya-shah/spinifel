...
def calc_eudist_gpu_sp(model_slices, slices, deviceId):
    ...
    euDist = pyCu_SP.cudaEuclideanDistance(
        model_slices_flat,
        slices_flat,
...
def calc_argmin_gpu_sp(euDist, n_images, n_refs, n_pixels, deviceId):
    ...
    index = pyCu_SP.cudaHeapSort(euDist, n_images, n_refs, n_pixels, 1, deviceId)
    return index
...
def calc_eudist_gpu_dp(model_slices, slices, deviceId):
    ...
    euDist = pyCu_DP.cudaEuclideanDistance(
        model_slices_flat,
        slices_flat,
        model_slices.shape[0],
        slices.shape[0],
...
def calc_argmin_gpu_dp(euDist, n_images, n_refs, n_pixels, deviceId):
    ...
    index = pyCu_DP.cudaHeapSort(euDist, n_images, n_refs, n_pixels, 1, deviceId)
    return index
...
def compare_cpu_gpu_knn():
    ...
    gpu_indices_sp = calc_argmin_gpu_sp(
        gpu_eu_dist_sp,
        slices.shape[0],
        model_slices.shape[0],
        slices.shape[1],
    ...
    gpu_indices_dp = calc_argmin_gpu_dp(
        gpu_eu_dist_dp,
        slices.shape[0],
        model_slices.shape[0],
        slices.shape[1],
