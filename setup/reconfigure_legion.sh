...
cmake -DCMAKE_PREFIX_PATH="$CONDA_ENV_DIR" \
    ...
    -DLegion_USE_OpenMP=$([ $USE_OPENMP -eq 1 ] && echo ON || echo OFF) \
    -DLegion_NETWORKS=$([ $USE_GASNET -eq 1 ] && echo gasnetex) \
    -DLegion_EMBED_GASNet=$([ $USE_GASNET -eq 1 ] && echo ON || echo OFF) \
